...
func TestHashSparkInvoiceFields_TokensBasic(t *testing.T) {
	...
	receiverPublicKey := identityPublicKey
...
func TestHashSparkInvoiceFields_InvalidInputs(t *testing.T) {
	...
	fMissingPT := &pb.SparkInvoiceFields{Version: 1, Id: id, Memo: &memo, SenderPublicKey: identityPublicKey.Serialize()}
...
func TestHashSparkInvoiceFields_EmptyAndNilEquivalences(t *testing.T) {
	...
	h1, err := HashSparkInvoiceFields(fNilTI, network, identityPublicKey)
	require.NoError(t, err)
	h2, err := HashSparkInvoiceFields(fEmptyTI, network, identityPublicKey)
	require.NoError(t, err)
	require.Equal(t, h1, h2)
	...
	h3, err := HashSparkInvoiceFields(fNilAmt, network, identityPublicKey)
	require.NoError(t, err)
	h4, err := HashSparkInvoiceFields(fEmptyAmt, network, identityPublicKey)
	require.NoError(t, err)
	...
	h5, err := HashSparkInvoiceFields(fNilSats, network, identityPublicKey)
	require.NoError(t, err)
	h6, err := HashSparkInvoiceFields(fZeroSats, network, identityPublicKey)
	require.NoError(t, err)
	...
	h7, err := HashSparkInvoiceFields(fMemoNil, network, identityPublicKey)
	require.NoError(t, err)
	h8, err := HashSparkInvoiceFields(fMemoEmpty, network, identityPublicKey)
	require.NoError(t, err)
	require.Equal(t, h7, h8)

	fSpkNil := CreateSatsSparkInvoiceFields(id, &zero, nil, keys.Public{}, nil)
	_, err = HashSparkInvoiceFields(fSpkNil, network, identityPublicKey)
	require.NoError(t, err)
}
...
func TestHashSparkInvoiceFieldsProducesKnownHash(t *testing.T) {
	receiverPublicKey := keys.MustParsePublicKeyHex("026c943bfef71040371ca1c1d1ee1d5b203573dc97fdf6497a0b74e5aec0220e21")
	tokenID, _ := hex.DecodeString("49046dd67bbe5fc8e3abb45bc4f809b9cb5cb5871a19292fa5c7120389641363")
