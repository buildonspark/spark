...
// The TokenAllowanceFunc type is an adapter to allow the use of ordinary
// function as TokenAllowance mutator.
type TokenAllowanceFunc func(context.Context, *ent.TokenAllowanceMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TokenAllowanceFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TokenAllowanceMutation); ok {
		return f(ctx, mv)
	}
