...
type TokenLeaf struct {
	...
	// LeafCreatedTransactionOuputVout holds the value of the "leaf_created_transaction_ouput_vout" field.
	LeafCreatedTransactionOuputVout uint32 `json:"leaf_created_transaction_ouput_vout,omitempty"`
...
func (*TokenLeaf) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case tokenleaf.FieldOwnerPublicKey, tokenleaf.FieldWithdrawalRevocationPublicKey, tokenleaf.FieldTokenPublicKey, tokenleaf.FieldTokenAmount, tokenleaf.FieldLeafSpentOwnershipSignature, tokenleaf.FieldLeafSpentRevocationPrivateKey:
			values[i] = new([]byte)
		case tokenleaf.FieldWithdrawalBondSats, tokenleaf.FieldWithdrawalLocktime, tokenleaf.FieldLeafCreatedTransactionOuputVout, tokenleaf.FieldLeafSpentTransactionInputVout:
...
func (tl *TokenLeaf) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case tokenleaf.FieldLeafCreatedTransactionOuputVout:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field leaf_created_transaction_ouput_vout", values[i])
			} else if value.Valid {
				tl.LeafCreatedTransactionOuputVout = uint32(value.Int64)
			}
...
func (tl *TokenLeaf) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("leaf_created_transaction_ouput_vout=")
	builder.WriteString(fmt.Sprintf("%v", tl.LeafCreatedTransactionOuputVout))
	builder.WriteString(", ")
