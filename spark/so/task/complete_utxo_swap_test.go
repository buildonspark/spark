...
// TestCompleteUtxoSwap_RefundConsensusKnob pins the sweep's interaction with the
// 2PC refund migration: the sweep force-completes a stale CREATED refund swap
// regardless of spark.so.use_consensus_static_deposit_utxo_refund. A CREATED
...
// task.go), and sweeping it keeps the legacy self-heal path alive across the
// knob flip: refund retries only re-sign against a COMPLETED swap, so an
// unswept stray would block the UTXO's active-swap slot indefinitely.
func TestCompleteUtxoSwap_RefundConsensusKnob(t *testing.T) {
	tests := []struct {
		name           string
		...
		{
			name:           "consensus refund knob off - stale refund swap is swept to COMPLETED",
			knobValues:     map[string]float64{},
			expectedStatus: st.UtxoSwapStatusCompleted,
		},
		{
			name:           "consensus refund knob on - legacy stray refund swap is still swept to COMPLETED",
			knobValues:     map[string]float64{knobs.KnobUseConsensusStaticDepositUtxoRefund: 1},
			expectedStatus: st.UtxoSwapStatusCompleted,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			err = task.RunOnce(ctx, cfg, client, nil, knobs.NewFixedKnobs(tc.knobValues))
			require.NoError(t, err)
