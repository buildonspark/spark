		...
		},
		{
			...
			BaseTaskSpec: BaseTaskSpec{
				...
				Task: func(ctx context.Context, config *so.Config, knobsService knobs.Knobs) error {
					...
					batch_size := int(knobsService.GetValue(knobs.KnobTransferBackfillBatchSize, 1000))

					// Query for transfers with NULL network, load their transfer leaves with tree nodes
					transfers, err := tx.Transfer.Query().
						Where(transfer.NetworkIsNil()).
						WithTransferLeaves(func(q *ent.TransferLeafQuery) {
							q.WithLeaf() // Load the tree node for each transfer leaf
						}).
						Limit(batch_size).
						All(ctx)
					...
					if len(transfers) == 0 {
						logger.Debug("No transfers to backfill")
						return nil
					}

					logger.Sugar().Infof("Found %d transfers to backfill network", len(transfers))

					// Group transfers by network for bulk updates
					networkGroups := make(map[st.Network][]uuid.UUID)
					unspecifiedIDs := []uuid.UUID{}

					for _, t := range transfers {
						// Check if transfer has no transfer leaves
						if len(t.Edges.TransferLeaves) == 0 {
							logger.Sugar().Warnf("Transfer %s has no transfer leaves, marking as UNSPECIFIED", t.ID)
							unspecifiedIDs = append(unspecifiedIDs, t.ID)
							continue
						}
						...
						for i, leaf := range t.Edges.TransferLeaves {
							if leaf.Edges.Leaf == nil {
								logger.Sugar().Warnf("Transfer %s has transfer leaf %d without tree node, marking as UNSPECIFIED", t.ID, i)
								hasIssue = true
								break
							}

							leafNetwork := leaf.Edges.Leaf.Network
							if leafNetwork == "" {
								logger.Sugar().Warnf("Transfer %s has tree node %s without network, marking as UNSPECIFIED", t.ID, leaf.Edges.Leaf.ID)
								hasIssue = true
								break
							}
							...
							if i == 0 {
								network = leafNetwork
							} else if network != leafNetwork {
								logger.Sugar().Errorf("Transfer %s has mixed networks: first leaf has %s, leaf %d has %s, marking as UNSPECIFIED",
									t.ID, network, i, leafNetwork)
								hasIssue = true
								break
							}
						}
						...
						// Group by network
						networkGroups[network] = append(networkGroups[network], t.ID)
					}

					// Perform bulk updates for each network
					processedCount := 0
					for network, ids := range networkGroups {
						affected, err := tx.Transfer.Update().
							Where(transfer.IDIn(ids...)).
							SetNetwork(network).
							Save(ctx)
						if err != nil {
							logger.Error("Failed to bulk update transfers for network", zap.String("network", string(network)), zap.Error(err))
							return fmt.Errorf("failed to bulk update transfers for network %s: %w", network, err)
						}
						processedCount += affected
						logger.Sugar().Infof("Updated %d transfers to network %s", affected, network)
					}

					// Mark problematic transfers as UNSPECIFIED so they won't be reprocessed
					if len(unspecifiedIDs) > 0 {
						affected, err := tx.Transfer.Update().
							Where(transfer.IDIn(unspecifiedIDs...)).
							SetNetwork(st.NetworkUnspecified).
							Save(ctx)
						if err != nil {
							logger.Error("Failed to mark transfers as UNSPECIFIED", zap.Error(err))
							return fmt.Errorf("failed to mark transfers as UNSPECIFIED: %w", err)
						}
						logger.Sugar().Warnf("Marked %d transfers as UNSPECIFIED due to missing or inconsistent data", affected)
					}

					logger.Sugar().Infof("Successfully backfilled %d transfers", processedCount)
					return nil
				},
			},
		},
