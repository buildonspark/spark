...
func TestStartTreeCreation(t *testing.T) {
	...
	client := pbspark.NewSparkServiceClient(conn)
	...
	treeResponse, _ := client.StartTreeCreation(context.Background(), &pbspark.StartTreeCreationRequest{
		IdentityPublicKey: userPubkey,
		OnChainUtxo: &pbspark.UTXO{
			Txid: depositTx.TxID(),
			Vout: uint32(vout),
		},
		RootTxSigningJob: &pbspark.SigningJob{
			RawTxHex:               hex.EncodeToString(rootBuf.Bytes()),
			SigningPublicKey:       userPubkey,
			SigningNonceCommitment: &rootNonceCommitment,
		},
		RefundTxSigningJob: &pbspark.SigningJob{
