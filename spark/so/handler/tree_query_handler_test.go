...
func TestQueryNodes_IncludeParents_CrossOwnerPrivacy(t *testing.T) {
	ctx, cfg := createTestContextWithKnobsBypassed(t)
	tx, err := ent.GetDbFromContext(ctx)
	require.NoError(t, err)
	rng := rand.NewChaCha8([32]byte{})

	// Two owners: attacker (public wallet) and victim (private wallet)
	attackerPubKey := keys.MustGeneratePrivateKeyFromRand(rng).Public()
	victimPubKey := keys.MustGeneratePrivateKeyFromRand(rng).Public()
	signingPubKey := keys.MustGeneratePrivateKeyFromRand(rng).Public()
	verifyingPubKey := keys.MustGeneratePrivateKeyFromRand(rng).Public()
	secretShare := keys.MustGeneratePrivateKeyFromRand(rng)
	...
	tree, err := tx.Tree.Create().
		SetOwnerIdentityPubkey(victimPubKey).
		SetNetwork(btcnetwork.Regtest).
		SetStatus(st.TreeStatusAvailable).
		SetBaseTxid(st.NewRandomTxIDForTesting(t)).
		SetVout(1).
	...
	// Parent node owned by victim (private wallet)
	parentNode, err := tx.TreeNode.Create().
		SetTree(tree).
		SetNetwork(tree.Network).
		SetStatus(st.TreeNodeStatusAvailable).
		SetOwnerIdentityPubkey(victimPubKey).
	...
	// Child node owned by attacker (public wallet), parented under victim's node
	childNode, err := tx.TreeNode.Create().
		SetTree(tree).
		SetParent(parentNode).
		SetNetwork(tree.Network).
		SetStatus(st.TreeNodeStatusAvailable).
		SetOwnerIdentityPubkey(attackerPubKey).
		SetOwnerSigningPubkey(signingPubKey).
		SetValue(100000).
		SetVerifyingPubkey(verifyingPubKey).
		SetSigningKeyshare(signingKeyshare).
	...
	// Victim enables privacy
	_, err = tx.WalletSetting.Create().
		SetOwnerIdentityPublicKey(victimPubKey).
		SetPrivateEnabled(true).
		Save(ctx)
	require.NoError(t, err)

	// Attacker's session
	ctx = authn.InjectSessionForTests(ctx, hex.EncodeToString(attackerPubKey.Serialize()), 9999999999)

	handler := NewTreeQueryHandler(cfg)

	// Query the attacker's own child node by ID with include_parents=true
	resp, err := handler.QueryNodes(ctx, &pb.QueryNodesRequest{
		Source: &pb.QueryNodesRequest_NodeIds{
			NodeIds: &pb.TreeNodeIds{
				NodeIds: []string{childNode.ID.String()},
			},
		},
		IncludeParents: true,
	}, false)
	...
	// The child node should be returned with its owner identity intact
	assert.Contains(t, resp.Nodes, childNode.ID.String(),
		"Attacker should see their own child node")
	childProto := resp.Nodes[childNode.ID.String()]
	assert.NotEmpty(t, childProto.OwnerIdentityPublicKey,
		"Directly queried child node should retain its owner identity")

	// The parent node should be returned but with owner identity stripped
	assert.Contains(t, resp.Nodes, parentNode.ID.String(),
		"Parent node should be included in the ancestor chain")
	parentProto := resp.Nodes[parentNode.ID.String()]
	assert.Empty(t, parentProto.OwnerIdentityPublicKey,
		"Ancestor node must not expose owner identity public key")
	assert.Empty(t, parentProto.OwnerSigningPublicKey,
		"Ancestor node must not expose owner signing public key")
}

func TestQueryNodes_IncludeParents_DirectQueryNotOverwritten(t *testing.T) {
	ctx, _ := db.NewTestSQLiteContext(t)
	tx, err := ent.GetDbFromContext(ctx)
	require.NoError(t, err)
	rng := rand.NewChaCha8([32]byte{})
	...
	ctx = authn.InjectSessionForTests(ctx, hex.EncodeToString(ownerPubKey.Serialize()), 9999999999)
	handler := NewTreeQueryHandler(&so.Config{})

	// Query both parent and child by ID with include_parents — the parent
	// is directly queried (owner keys intact) and also an ancestor of child.
	// The ancestor chain must not overwrite the directly-queried entry.
	resp, err := handler.QueryNodes(ctx, &pb.QueryNodesRequest{
		Source: &pb.QueryNodesRequest_NodeIds{
			NodeIds: &pb.TreeNodeIds{
				NodeIds: []string{parentNode.ID.String(), childNode.ID.String()},
			},
		},
		IncludeParents: true,
	}, false)
	...
	parentProto := resp.Nodes[parentNode.ID.String()]
	require.NotNil(t, parentProto)
	assert.NotEmpty(t, parentProto.OwnerIdentityPublicKey,
		"Directly queried parent must retain owner identity even with include_parents")
	assert.NotEmpty(t, parentProto.OwnerSigningPublicKey,
		"Directly queried parent must retain owner signing key even with include_parents")
}
