...
func (h *LightningHandler) GetPreimageShare(
	...
	validateErr := func() error {
		...
		if preimageShare != nil {
			...
			// 2PC flow handler's coordinator path.
...
// driven through the 2PC consensus engine (initiatePreimageSwapV3Consensus).
// V3 semantics: requireDirectTx=true, no expiry override (expiry is taken from
// the transfer_request as-is).
