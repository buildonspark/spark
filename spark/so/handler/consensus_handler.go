...
func consensusFlowHandler(config *so.Config, opType pbgossip.ConsensusOperationType) (consensus.FlowHandler, error) {
	...
	case pbgossip.ConsensusOperationType_CONSENSUS_OPERATION_TYPE_INITIATE_PREIMAGE_SWAP_V4:
		return NewInitiatePreimageSwapV4FlowHandler(config), nil
	...
	case pbgossip.ConsensusOperationType_CONSENSUS_OPERATION_TYPE_TOKEN_TRANSACTION:
		return tokenhandler.NewTokenTransactionFlowHandler(config), nil
	default:
