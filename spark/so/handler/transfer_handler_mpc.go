...
// the gates below, then fails closed with Unimplemented — the flow that verifies the group signature and authorized
// facts (SP-3505) and executes through the consensus engine (SP-3506) lands behind the same knob. Nothing here can
// commit a transfer.
//
...
func (h *TransferHandler) StartTransferMpc(ctx context.Context, req *pb.StartTransferMpcRequest) (*pb.StartTransferResponse, error) {
	...
	// Fail-closed tail: the submission is structurally acceptable, but operator-side verification and the consensus
	// flow for CONSENSUS_OPERATION_TYPE_MPC_SEND_TRANSFER are not implemented yet. Refusing here (rather than not
