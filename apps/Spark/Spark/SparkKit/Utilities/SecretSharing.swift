...
struct Polynomial {
    ...
    init(fieldModulus: BigInt, secret: BigInt, degree: UInt32) throws {
        ...
        proofs.append(try secp256k1.Signing.PrivateKey(dataRepresentation: secret.serialize().padTo32Bytes()).publicKey.dataRepresentation)
...
public func splitSecret(fieldModulus: BigInt, secret: BigInt, threshold: UInt32, numberOfShares: UInt32) throws -> [VerifiableSecretShare] {
    ...
    for i in 1...numberOfShares {
        ...
        result.append(
            VerifiableSecretShare(fieldModulus: fieldModulus, threshold: threshold, index: BigInt(i), share: share, proof: poly.proofs)
        )
    }
    return result
}
...
func computeLagrangeCoefficients(index: BigInt, shares: [VerifiableSecretShare]) throws -> BigInt{
    ...
    for share in shares {
        ...
        denominator = (denominator * ((share.index - index + share.fieldModulus) % share.fieldModulus)) % share.fieldModulus
    }
