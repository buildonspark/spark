...
describe("coop exit", () => {
    ...
    testFn("test coop exit", async () => {
        ...
        const withdrawAddressScript = getP2TRScriptFromPublicKey(withdrawPubKey, Network.LOCAL);
        const leafCount = 1;
        ...
        const intermediateOutPoint = {
            txid: hexToBytes(exitTxId),
            index: 1,
        };
        let connectorP2trAddrs = [];
        for (let i = 0; i < leafCount + 1; i++) {
            const connectorPubKey = await userWallet
                .getSigner()
                .generatePublicKey();
            const connectorP2trAddr = getP2TRAddressFromPublicKey(connectorPubKey, Network.LOCAL);
        ...
        const leafPubKeyMap = await sspWallet.verifyPendingTransfer(receiverTransfer);
        expect(leafPubKeyMap.size).toBe(1);
        expect(leafPubKeyMap.get(rootNode.id)).toBeDefined();
        expect(equalBytes(leafPubKeyMap.get(rootNode.id), newLeafPubKey)).toBe(true);
        ...
        const signedExitTx = await faucet.signFaucetCoin(exitTx, faucetCoin.txout, faucetCoin.key);
