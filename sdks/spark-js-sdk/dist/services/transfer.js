import { bytesToHex, equalBytes, numberToBytesBE, } from "@noble/curves/abstract/utils";
...
export class BaseTransferService {
    ...
    async sendTransferTweakKey(transfer, leaves, refundSignatureMap) {
        ...
        let updatedTransfer;
        const errors = [];
        const promises = Object.entries(this.config.getConfig().signingOperators).map(async ([identifier, operator]) => {
    ...
    async prepareSingleSendTransferKeyTweak(transferID, leaf, receiverEciesPubKey, refundSignature) {
        ...
        for (const [identifier, operator] of Object.entries(this.config.getConfig().signingOperators)) {
...
export class TransferService extends BaseTransferService {
    ...
    async sendTransfer(leaves, receiverIdentityPubkey, expiryTime) {
        const { transfer, signatureMap } = await this.sendTransferSignRefund(leaves, receiverIdentityPubkey, expiryTime);
    ...
    prepareRefundSoSigningJobs(leaves, leafDataMap) {
        ...
        for (const leaf of leaves) {
            ...
            const { refundTx } = createRefundTx(leaf.leaf, refundSigningData.receivingPubkey, this.config.getNetwork());
