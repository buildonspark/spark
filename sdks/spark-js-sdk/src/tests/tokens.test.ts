...
import { hashTokenTransaction, hashOperatorSpecificTokenTransactionSignablePayload } from '../utils/tokens';
...
describe('hash token transaction', () => {
  it ('should produce the exact same hash', () => {
    ...
    const tokenPublicKey = new Uint8Array([
      242, 155, 208, 90, 72, 211, 120, 244,
      69, 99, 28, 101, 149, 222, 123, 50,
      252, 63, 99, 54, 137, 226, 7, 224,
      163, 122, 93, 248, 42, 159, 173, 45
    ]);

    const identityPubKey = new Uint8Array([
      25, 155, 208, 90, 72, 211, 120, 244,
      69, 99, 28, 101, 149, 222, 123, 50,
      252, 63, 99, 54, 137, 226, 7, 224,
      163, 122, 93, 248, 42, 159, 173, 46
    ]);
    ...
    expect(Array.from(hash)).toEqual([142, 183, 62, 229, 88, 150, 67, 230, 159, 27, 221, 120, 221, 0, 49, 134, 139, 11, 249, 227, 197, 145, 122, 52, 136, 189, 98, 172, 53, 238, 118, 72]);
  });
  ...
  it ('should produce the exact same hash part 2', () => {
    ...
    expect(Array.from(hash)).toEqual([118, 30, 129, 21, 204, 132, 45, 226, 127, 114, 43, 4, 153, 100, 234, 235, 220, 121, 8, 145, 143, 219, 168, 222, 31, 159, 175, 121, 78, 87, 134, 87]);
  });
});
...
describe('hash operator specific token transaction signable payload', () => {
  it('should produce consistent hashes', () => {
      ...
      const hash1 = hashOperatorSpecificTokenTransactionSignablePayload(testCase1);
      ...
      const hash2 = hashOperatorSpecificTokenTransactionSignablePayload(testCase2);
      ...
      const hash3 = hashOperatorSpecificTokenTransactionSignablePayload(testCase3);
  });
  ...
  it('should produce the exact same hash', () => {
    ...
    expect(Array.from(hash)).toEqual([
      24, 134, 89, 30, 173, 70, 40, 117, 
      169, 133, 82, 81, 219, 74, 39, 126, 
      186, 174, 255, 170, 79, 215, 9, 111, 
      63, 8, 15, 123, 212, 247, 134, 70
    ]);
  });
});