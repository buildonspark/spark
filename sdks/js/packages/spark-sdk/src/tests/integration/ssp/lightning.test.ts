...
import { BitcoinNetwork, CurrencyUnit, LightningReceiveRequestStatus } from "../../../types/index.js";

const options: ConfigOptions = {
  network: "LOCAL",
};
const { wallet: walletStatic, ...rest } =
  await SparkWallet.initialize({
    mnemonicOrSeed: "logic ripple layer execute smart disease marine hero monster talent crucial unfair horror shadow maze abuse avoid story loop jaguar sphere trap decrease turn",
    options,
  });
...
describe("Lightning Network provider", () => {
  describe("should create lightning invoice", () => {
    test.concurrent.each([
      ...
      expect(invoice.invoice.encodedEnvoice).toMatch(new RegExp(`^${invoicePrefix}[0-9]*[a-zA-Z0-9]{400}$`));
      expect(invoice.invoice.paymentHash.length).toEqual(64);
      expect(invoice.invoice.amount.originalValue).toEqual(1000 * 1000);
      expect(invoice.invoice.amount.originalUnit).toEqual(CurrencyUnit.MILLISATOSHI);
      expect(invoice.status).toEqual(LightningReceiveRequestStatus.INVOICE_CREATED);
      expect(invoice.transfer).toBeUndefined();
      expect(invoice.invoice.bitcoinNetwork).toEqual(network as BitcoinNetwork);
    ...
    test.concurrent.each([
      ...
      expect(invoice.invoice.encodedEnvoice.length).toBeGreaterThanOrEqual(401);
      expect(invoice.invoice.paymentHash.length).toEqual(64);
  ...
  describe("should fail to create lightning invoice", () => {
    it(`should fail to create lightning invoice with invalid amount`, async () => {
      await expect(walletStatic.createLightningInvoice({
        amountSats: -1,
        memo: "test",
      })).rejects.toThrow();
    }, 30000);
